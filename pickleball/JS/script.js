...
function startWatchingPosition(court, user) {
    ...
    const watcherId = navigator.geolocation.watchPosition(
        (position) => {
            ...
            if (distance > 0.5) {
                showNotification(`Você se afastou mais de 500 metros da quadra ${court.toUpperCase()} e foi removido automaticamente.`);
...
async function handleRegister(event) {
    ...
    const playsAt = document.getElementById("register-plays-at").value.trim().toUpperCase();
...
function sendMessage() {
    ...
    chatRef.on("child_added", async (snapshot) => {
        ...
        // AQUI ESTÁ A VERIFICAÇÃO SE É MENSAGEM PRÓPRIA
        const isOwnMessage = currentUser && message.senderUid === currentUser.uid;
        ...
        const contentSpan = document.createElement("span"); // Crie o contentSpan aqui
        contentSpan.classList.add("content");
        contentSpan.textContent = message.text;

        // ADICIONE/MANTENHA ESTA LÓGICA AQUI:
        if (!isOwnMessage) {
            // Lógica para mensagens de OUTROS usuários (com nome)
            ...
            senderSpan.textContent = `${finalSenderName} (${formattedTime}): `;
            messageElement.appendChild(senderSpan); // Adiciona o nome antes do conteúdo
            messageElement.appendChild(contentSpan); // Adiciona o conteúdo
        } else {
            // Lógica para SUAS PRÓPRIAS mensagens (sem nome, apenas conteúdo ou horário)
            // Se você quiser que NADA apareça antes da sua mensagem, remova este bloco 'else' inteiro.
            // Se quiser apenas o horário, mantenha assim:
            const timeSpan = document.createElement("span");
            timeSpan.classList.add("sender"); // Reutiliza a classe sender para estilização
            timeSpan.textContent = `(${formattedTime})`;
            messageElement.appendChild(timeSpan); // Adiciona o horário
            messageElement.appendChild(contentSpan); // Adiciona o conteúdo
        }

        // REMOVA ESTAS LINHAS (já foram tratadas acima)
...
async function showPlayerDetailsModal(playerUid, playerProfile) {
    ...
    try {
        if (playerProfile) {
            ...
            document.getElementById("player-detail-gender").textContent = playerProfile.gender || "N/A";
            document.getElementById("player-detail-plays-at").textContent = playerProfile.playsAt || "N/A";
...
// NOVO: Função para trocar de aba no modal de configurações
function openSettingsTab(evt, tabName) {
    ...
    // Carrega os dados relevantes ao abrir a aba de Amigos
    if (tabName === 'Friends') {
        loadFriendRequests();
        loadFriendsList();
    }
...
// NOVO: Função para adicionar um amigo
async function sendFriendRequest() {
    const friendConfigId = document.getElementById('add-friend-id').value.trim();
    if (!friendConfigId) {
        showNotification("Por favor, insira o ID de configuração do amigo.");
        return;
    }
    ...
    if (friendConfigId === currentUser.configId) {
        showNotification("Você não pode enviar uma solicitação para si mesmo.");
        return;
    }

    const usersRef = firebase.database().ref('users');
    const snapshot = await usersRef.orderByChild('configId').equalTo(friendConfigId).once('value');
    ...
    const areFriendsCheck = await firebase.database().ref(`users/${currentUser.uid}/friends/${friendUid}`).once('value');
    if (areFriendsCheck.exists()) {
        showNotification(`Você já é amigo de ${friendProfile.username}.`);
        return;
    }

    // Verificar se já existe uma solicitação pendente
    const requestSentCheck = await firebase.database().ref(`friend_requests/${friendUid}/received/${currentUser.uid}`).once('value');
    if (requestSentCheck.exists()) {
        showNotification("Você já enviou uma solicitação para este jogador.");
        return;
    }
    const requestReceivedCheck = await firebase.database().ref(`friend_requests/${currentUser.uid}/received/${friendUid}`).once('value');
    if (requestReceivedCheck.exists()) {
        showNotification("Este jogador já te enviou uma solicitação. Verifique suas solicitações pendentes.");
        return;
    }
    ...
    const requestData = {
        ...
        fromName: `${currentUser.username} ${currentUser.lastName || ''}`.trim(),
        timestamp: firebase.database.ServerValue.TIMESTAMP
    };

    // Salva a solicitação no nó do destinatário
    await firebase.database().ref(`friend_requests/${friendUid}/received/${currentUser.uid}`).set(requestData);
    
    // (Opcional) Salva um registro da solicitação enviada no nó do remetente
    await firebase.database().ref(`friend_requests/${currentUser.uid}/sent/${friendUid}`).set({
        toName: `${friendProfile.username} ${friendProfile.lastName || ''}`.trim(),
        timestamp: firebase.database.ServerValue.TIMESTAMP
    });

    showNotification(`Solicitação de amizade enviada para ${friendProfile.username}!`);
    document.getElementById('add-friend-id').value = '';
}

// NOVO: Função para carregar solicitações de amizade pendentes
async function loadFriendRequests() {
    const currentUser = getCurrentUser();
    if (!currentUser) return;

    const requestsListElement = document.getElementById('friend-requests-list');
    requestsListElement.innerHTML = '<li>Verificando solicitações...</li>';

    const requestsRef = firebase.database().ref(`friend_requests/${currentUser.uid}/received`);
    requestsRef.on('value', (snapshot) => {
        const requests = snapshot.val();
        if (!requests) {
            requestsListElement.innerHTML = '<li>Nenhuma solicitação pendente.</li>';
            return;
        }
        ...
        for (const senderUid in requests) {
            ...
            listItem.innerHTML = `
                <span class="request-info">${request.fromName} quer ser seu amigo.</span>
                <div class="request-actions">
                    <button class="accept-request-btn" onclick="acceptFriendRequest('${senderUid}')" title="Aceitar">✓</button>
                    <button class="reject-request-btn" onclick="rejectFriendRequest('${senderUid}')" title="Recusar">×</button>
                </div>
            `;
            requestsListElement.appendChild(listItem);
        }
    });
}

// NOVO: Função para aceitar uma solicitação de amizade
async function acceptFriendRequest(senderUid) {
    const currentUser = getCurrentUser();
    if (!currentUser) return;

    // Obter perfis de ambos os usuários para salvar informações completas
    const senderProfile = await getUserProfile(senderUid);
    const currentUserProfile = await getUserProfile(currentUser.uid);

    if (!senderProfile || !currentUserProfile) {
        showNotification("Erro ao encontrar perfis. Tente novamente.");
        return;
    }

    // Adicionar um ao outro na lista de amigos
    const updates = {};
    updates[`/users/${currentUser.uid}/friends/${senderUid}`] = {
        uid: senderUid,
        username: senderProfile.username,
        lastName: senderProfile.lastName,
        configId: senderProfile.configId
    };
    updates[`/users/${senderUid}/friends/${currentUser.uid}`] = {
        uid: currentUser.uid,
        username: currentUserProfile.username,
        lastName: currentUserProfile.lastName,
        configId: currentUserProfile.configId
    };

    // Remover a solicitação dos nós 'received' e 'sent'
    updates[`/friend_requests/${currentUser.uid}/received/${senderUid}`] = null;
    updates[`/friend_requests/${senderUid}/sent/${currentUser.uid}`] = null;

    await firebase.database().ref().update(updates);

    showNotification(`Você e ${senderProfile.username} agora são amigos!`);
    loadFriendsList(); // Recarrega a lista de amigos
}

// NOVO: Função auxiliar para mostrar detalhes do jogador a partir da lista de amigos
async function showPlayerDetailsModalByUid(playerUid) {
    const playerProfile = await getUserProfile(playerUid);
    if (playerProfile) {
        showPlayerDetailsModal(playerUid, playerProfile);
    } else {
        showNotification("Não foi possível carregar os detalhes deste jogador.");
    }
}


// Modificar a função showUserSettingsModal para resetar para a primeira aba
async function acceptFriendRequest(senderUid) {
    const currentUser = getCurrentUser();
    if (!currentUser) return;
...
// NOVO: Função para recusar uma solicitação de amizade
async function rejectFriendRequest(senderUid) {
    ...
    await firebase.database().ref().update(updates);
    showNotification("Solicitação de amizade recusada.");
}


// MODIFICADO: A função agora é chamada ao abrir a aba de amigos
async function loadFriendsList() {
    const currentUser = getCurrentUser();
    if (!currentUser) return;

    const friendsListElement = document.getElementById('friends-list');
    // Adiciona uma classe para estilizar o estado de carregamento
    friendsListElement.innerHTML = '<li class="loading-state">Carregando amigos...</li>'; 

    const friendsRef = firebase.database().ref(`users/${currentUser.uid}/friends`);
    friendsRef.on('value', (snapshot) => {
        const friends = snapshot.val();
        if (!friends) {
            // Adiciona uma classe para estilizar o estado vazio
            friendsListElement.innerHTML = '<li class="empty-state">Você ainda não adicionou nenhum amigo.</li>'; 
            return;
        }
        ...
        for (const friendUid in friends) {
            ...
            const listItem = document.createElement('li');
            listItem.className = 'friend-card'; // Nova classe para o card do amigo

            // Gerar iniciais para o avatar
            const initials = (friend.username ? friend.username[0] : '') + (friend.lastName ? friend.lastName[0] : '');
            
            listItem.innerHTML = `
                <div class="friend-avatar" style="background-color: ${generateAvatarColor(friendUid)}">${initials.toUpperCase()}</div>
                <div class="friend-details">
                    <span class="friend-name" onclick="showPlayerDetailsModalByUid('${friend.uid}')">${friend.username} ${friend.lastName || ''}</span>
                    <span class="friend-id">ID: ${friend.configId}</span>
                </div>
                <button class="remove-friend-btn" onclick="removeFriend('${friend.uid}')" title="Remover amigo">×</button>
            `;
...
// NOVO: Função auxiliar para gerar uma cor consistente para o avatar com base no UID
function generateAvatarColor(uid) {
    let hash = 0;
    for (let i = 0; i < uid.length; i++) {
        hash = uid.charCodeAt(i) + ((hash << 5) - hash);
    }
    // Usando HSL para cores mais agradáveis e suaves (tons pastel)
    const color = `hsl(${hash % 360}, 70%, 85%)`;
    return color;
}

// MODIFICADO: Agora remove a amizade de ambos os usuários
async function removeFriend(friendUid) {
    if (!confirm("Tem certeza que deseja remover este amigo? Esta ação também removerá você da lista de amigos dele.")) {
        return;
    }
    ...
    // Reset para a primeira aba (Detalhes) sempre que o modal for aberto
    openSettingsTab({ currentTarget: document.querySelector('.tab-link') }, 'Details');
    ...
    const userSessionsRef = database.ref(`userSessions/${currentUser.uid}`);
    const snapshot = await userSessionsRef.orderByChild('startTime').limitToLast(20).once("value"); // Limita aos últimos 20 para performance
    const sessionsData = snapshot.val();

    if (sessionsData) {
        const sessionsArray = Object.values(sessionsData).sort((a, b) => b.startTime - a.startTime);
